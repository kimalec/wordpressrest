...
var WordPressRest = function(options) {
	...
	var wurl = url.parse((reqisstring ? req : req.url), true);
	if(form) form = querystring.stringify(form);
	if(!reqisstring) {
	...
	var clientrequest = (wurl.protocol == 'https:' ? https : http).request({
	    ...
	    'path': wurl.path,
	    'headers': (reqisstring ? null : req.headers),
	    'agent': proxy
	});
	if(reqisstring) {
	...
	if(wurl.query.error) {
	    ...
	    this.request(url.format(wurl), post, callback);
	}
    };
