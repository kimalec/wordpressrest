...
var WordPressRest = function(options) {
    ...
    this.call_wordpress_authorize = function(req, res) {
	    ...
	    url_object.query = {'client_id': this.client_id,
				...
				'response_type': 'code'};
	    request({'uri': url.format(url_object), 'proxy': this.proxy}, function(error, response, body) {
		if(error) throw JSON.stringify(error);
		if(response.statusCode != 200) throw JSON.stringify(response);
		res.end(JSON.stringify({'type': 'html', 'html': body}));
	    }.bind(this));
    };
	...
	dbfilecontent.on('finish', function(dbfilecontent, req, res) {
		...
		res.end(JSON.stringify({'type': 'json', 'json': JSON.stringify({'status': 'authorized'})}));
	    }
	    else {
	...
	if(!code) {
	    res.end(JSON.stringify({'type': 'json', 'json': JSON.stringify({'status': 'denied'})}))
	}
	else {
	    url_object.protocol = this.protocol;
	    url_object.host = this.host;
	...
	request(request_params, function(req, res, error, response, body) {
            if(error) throw JSON.stringify(error);
            if(response.statusCode != 200) throw JSON.stringify(response);
            res.end(JSON.stringify({'type': 'json', 'json': body || JSON.stringify({})}));
	}.bind(this, req, res));
    };
