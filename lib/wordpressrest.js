...
var WordPressRest = function(options) {
	...
	var url_object = url.parse(pathname, true);
	var request_params = null;

	url_object.protocol = url_object.protocol || this.wordpress_protocol;
	url_object.host = url_object.host || this.wordpress_host;
	...
	if(req.method == 'GET') {
		...
		for(var k in options) {
		    url_object.query[k] = options[k];
		}
	    }
	    else {
		url_object.query = options;
	    }
	}
	if(req.method == 'POST') {
	    request_params.form = options;
	}
